Shader "Fraktalia/Core/MultiTexture/MultiTextureArray"
{
    Properties
    {
       [Header(Main Maps)]
    ...
    SubShader
    {
        ...
        UNITY_DECLARE_TEX2DARRAY(_MetallicMaps);
        UNITY_DECLARE_TEX2DARRAY(_OcclusionMaps);
        UNITY_DECLARE_TEX2DARRAY(_EmmissionMaps);
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            _SliceRange += (blendvalue + _BlendShift);
            ...
            float height1 = UNITY_SAMPLE_TEX2DARRAY(_HeightMaps, uv_texture1).r * (1 - blendfactor);
            float height2 = UNITY_SAMPLE_TEX2DARRAY(_HeightMaps, uv_texture2).r * (blendfactor);
            uv_texture1.xy += ParallaxOffset(height1 + height2, _HeightScale, IN.viewDir);
            uv_texture2.xy += ParallaxOffset(height1 + height2, _HeightScale, IN.viewDir);
            ...
            half4 bump1 = UNITY_SAMPLE_TEX2DARRAY(_NormalMaps, uv_texture1) * (1 - blendfactor);
            half4 bump2 = UNITY_SAMPLE_TEX2DARRAY(_NormalMaps, uv_texture2) * (blendfactor);
            o.Normal = UnpackScaleNormalArray(bump1 + bump2, _BumpScale);

            half4 m1 = UNITY_SAMPLE_TEX2DARRAY(_MetallicMaps, uv_texture1);
            half4 m2 = UNITY_SAMPLE_TEX2DARRAY(_MetallicMaps, uv_texture2);
            m1 *= (1 - blendfactor);
            m2 *= (blendfactor);
            ...
            fixed4 o1 = UNITY_SAMPLE_TEX2DARRAY(_OcclusionMaps, uv_texture1) * (1 - blendfactor);
            fixed4 o2 = UNITY_SAMPLE_TEX2DARRAY(_OcclusionMaps, uv_texture2) * (blendfactor);
            o.Occlusion = o1.r + o2.r;

            fixed4 e1 = UNITY_SAMPLE_TEX2DARRAY(_EmmissionMaps, uv_texture1) * (1 - blendfactor);
            fixed4 e2 = UNITY_SAMPLE_TEX2DARRAY(_EmmissionMaps, uv_texture2) * (blendfactor);
            o.Emission = e1 + e2;
