Shader "Fraktalia/Core/MultiTexture/MultiTextureArray_UVBlend_Tesselation_Triplanar_X4"
{
    Properties
    {
...
#pragma shader_feature SEAMLESS_TESSELATION
        ...
        #define GET_FROMTEXTUREARRAY_TRIPLANAR_LOD(tex, uvxy1, uvxz1, uvyz1, bf, output) \
        { \
            float4 o1 = UNITY_SAMPLE_TEX2DARRAY_LOD(tex, uvxy1,0); \
            float4 result1 = (o1)  * bf.z; \
            o1 = UNITY_SAMPLE_TEX2DARRAY_LOD(tex, uvxz1,0); \
            float4 result2 = (o1)  * bf.y; \
            o1 = UNITY_SAMPLE_TEX2DARRAY_LOD(tex, uvyz1,0); \
            float4 result3 = (o1)  * bf.x; \
            output = result1 + result2 + result3; \
        }
        ...
        float CalculateTesselationLayer(float2 trix, float2 triy, float2 triz, float3 bf, float power, float textureindex)
        {
            ...
            GET_FROMTEXTUREARRAY_TRIPLANAR_LOD(_ParallaxMap, xy1, xz1, yz1, bf, result);
            return result.r * power;
        }
        ...
        void disp(inout appdata_full v)
        {
            ...
            float layer1factor = clamp(0, 1, uv3_IndexTex.x * _UV3Power);
            float layer2factor = clamp(0, 1, uv3_IndexTex.y * _UV4Power);
            float layer3factor = clamp(0, 1, uv4_IndexTex.x * _UV5Power);
            float layer4factor = clamp(0, 1, uv4_IndexTex.y * _UV6Power);
            ...
            float base = CalculateTesselationLayer(trix, triy, triz, bf, layer0factor, _BaseSlice);
            float layeruv3 = CalculateTesselationLayer(trix, triy, triz, bf, layer1factor, _UV3Slice);
            float layeruv4 = CalculateTesselationLayer(trix, triy, triz, bf, layer2factor, _UV4Slice);
            float layeruv5 = CalculateTesselationLayer(trix, triy, triz, bf, layer3factor, _UV5Slice);
            float layeruv6 = CalculateTesselationLayer(trix, triy, triz, bf, layer4factor, _UV6Slice);
            float displacement = base + layeruv3 + layeruv4 + layeruv5 + layeruv6;
        ...
        Result CalculateLayer(Input IN, float2 trix, float2 triy, float2 triz, float3 bf, float power, float textureindex)
        {
            ...
            xy1.xy += ParallaxOffset(parallaxZ.r, _Parallax, viewDir) * power;
            xz1.xy += ParallaxOffset(parallaxY.r, _Parallax, viewDir) * power;
            yz1.xy += ParallaxOffset(parallaxX.r, _Parallax, viewDir) * power;
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            Result base = CalculateLayer(IN, trix, triy, triz, bf, layer0factor, _BaseSlice);
