...
{
    internal unsafe struct FStructLayoutData4
    {
        ...
        public FStructLayoutData4(Type t)
        {
    ...
    public unsafe struct FNativeArrayChunked8<T> : IDisposable where T : struct
    {
        private static FStructLayoutData4 ms_CachedLayout;
        ...
        public FNativeArrayChunked8(int length, Allocator label)
            : this(length, label, 2)
        {
        }

        FNativeArrayChunked8(int length, Allocator label, int disposeSentinelStackDepth)
        {
            FCollectionHelper.CheckIsUnmanaged<T>();
            if (!ms_CachedLayout.IsCreated)
            {
                ms_CachedLayout = new FStructLayoutData4(typeof(T));
            }

            m_Base = (byte*)UnsafeUtility.Malloc(FStructLayoutData4.ChunkSizeBytes * ms_CachedLayout.ChunksNeeded(length) * ms_CachedLayout.FieldCount, FStructLayoutData4.ChunkSizeBytes, label);
            m_Length = length;
            m_Allocator = label;
        ...
        public T this[int index]
        {
            get
            {
                ...
                uint* bp = (uint*)(m_Base + FStructLayoutData4.ChunkSizeBytes * ms_CachedLayout.FieldCount * chunkIndex + FStructLayoutData4.ElementSize * chunkOffset);
                ...
                for (int field = 0; field < fieldCount; ++field)
                {
                    ...
                    bp += FStructLayoutData4.ElementsPerChunk;
                }
            ...
            set
            {
                ...
                uint* target = (uint*)(m_Base + FStructLayoutData4.ChunkSizeBytes * ms_CachedLayout.FieldCount * chunkIndex + FStructLayoutData4.ElementSize * chunkOffset);

                for (int field = 0; field < fieldCount; ++field)
                {
                    var fieldInfo = ms_CachedLayout.GetFieldInfo(field);
                    *target = bp[fieldInfo.Offset / 4];
                    target += FStructLayoutData4.ElementsPerChunk;
                }
            }
        }
    }

    public unsafe struct FNativeArrayFullSOA<T> : IDisposable where T : struct
    {
        private static FStructLayoutData4 ms_CachedLayout;
        ...
        public FNativeArrayFullSOA(int length, Allocator label)
            : this(length, label, 2)
        {
        }

        FNativeArrayFullSOA(int length, Allocator label, int disposeSentinelStackDepth)
        {
            FCollectionHelper.CheckIsUnmanaged<T>();
            if (!ms_CachedLayout.IsCreated)
            {
            ...
            m_Base = (byte*)UnsafeUtility.Malloc(4 * length * ms_CachedLayout.FieldCount, FStructLayoutData4.ChunkSizeBytes, label);
            m_Length = length;
            m_Allocator = label;
