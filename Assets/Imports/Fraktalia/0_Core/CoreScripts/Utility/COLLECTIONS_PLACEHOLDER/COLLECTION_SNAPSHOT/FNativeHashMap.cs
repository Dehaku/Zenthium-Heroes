...
{
    public struct FNativeMultiHashMapIterator<TKey>
        where TKey : struct
    {
        internal TKey key;
    ...
    internal unsafe struct FNativeHashMapData
    {
        [FieldOffset(0)]
        public byte* values;
        ...
        internal static void IsBlittableAndThrow<TKey, TValue>()
            ...
            where TValue : struct
        {
            FCollectionHelper.CheckIsUnmanaged<TKey>();
            FCollectionHelper.CheckIsUnmanaged<TValue>();
        }

        public static void AllocateHashMap<TKey, TValue>(int length, int bucketLength, Allocator label,
            out FNativeHashMapData* outBuf)
            where TKey : struct
            where TValue : struct
        {
            IsBlittableAndThrow<TKey, TValue>();

            FNativeHashMapData* data = (FNativeHashMapData*)UnsafeUtility.Malloc(sizeof(FNativeHashMapData),
                UnsafeUtility.AlignOf<FNativeHashMapData>(), label);

            bucketLength = FCollectionHelper.CeilPow2(bucketLength);
        ...
        public static void ReallocateHashMap<TKey, TValue>(FNativeHashMapData* data, int newCapacity,
            int newBucketCapacity, Allocator label)
            where TKey : struct
            where TValue : struct
        {
            newBucketCapacity = FCollectionHelper.CeilPow2(newBucketCapacity);
        ...
        public static void DeallocateHashMap(FNativeHashMapData* data, Allocator allocation)
        {
        ...
        public static unsafe void GetKeyArray<TKey>(FNativeHashMapData* data, NativeArray<TKey> result)
            where TKey : struct
        {
        ...
        public static unsafe void GetValueArray<TValue>(FNativeHashMapData* data, NativeArray<TValue> result)
            where TValue : struct
        {
            var bucketArray = (int*)data->buckets;
            var bucketNext = (int*)data->next;
    ...
    internal struct NativeHashMapBase<TKey, TValue>
        ...
        where TValue : struct
    {
        public static unsafe void Clear(FNativeHashMapData* data)
        {
            int* buckets = (int*)data->buckets;
            for (int i = 0; i <= data->bucketCapacityMask; ++i)
                buckets[i] = -1;
            ...
            for (int tls = 0; tls < JobsUtility.MaxJobThreadCount; ++tls)
                data->firstFreeTLS[tls * FNativeHashMapData.IntsPerCacheLine] = -1;
            data->allocatedIndexLength = 0;
        }

        private static unsafe int AllocEntry(FNativeHashMapData* data, int threadIndex)
        {
            int idx;
            int* nextPtrs = (int*)data->next;
            do
            {
                idx = data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine];
                if (idx < 0)
                {
                    // Try to refill local cache
                    Interlocked.Exchange(ref data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine], -2);
                    ...
                    if (data->allocatedIndexLength < data->keyCapacity)
                    {
                        ...
                        {
                            int count = System.Math.Min(16, data->keyCapacity - idx);
                            for (int i = 1; i < count; ++i)
                                nextPtrs[idx + i] = idx + i + 1;
                            nextPtrs[idx + count - 1] = -1;
                            nextPtrs[idx] = -1;
                            Interlocked.Exchange(
                                ref data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine], idx + 1);
                            return idx;
                        }
                        ...
                        {
                            Interlocked.Exchange(
                                ref data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine], -1);
                            return idx;
                        }
                    }
                    Interlocked.Exchange(ref data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine], -1);
                    ...
                    while (again)
                    {
                            ...
                            other = (other + 1) % JobsUtility.MaxJobThreadCount)
                        {
                            do
                            {
                                idx = data->firstFreeTLS[other * FNativeHashMapData.IntsPerCacheLine];
                                if (idx < 0)
                                    break;
                            } while (Interlocked.CompareExchange(
                                         ref data->firstFreeTLS[other * FNativeHashMapData.IntsPerCacheLine],
                ...
                if (idx >= data->keyCapacity)
                {
                    throw new InvalidOperationException(string.Format("nextPtr idx {0} beyond capacity {1}", idx,
                        data->keyCapacity));
                         ...
                         ref data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine], nextPtrs[idx],
                         idx) != idx);
            nextPtrs[idx] = -1;
            return idx;
        }

        public static unsafe bool TryAddAtomic(FNativeHashMapData* data, TKey key, TValue item, int threadIndex)
        {
            TValue tempItem;
            FNativeMultiHashMapIterator<TKey> tempIt;
            ...
            if (Interlocked.CompareExchange(ref buckets[bucket], idx, -1) != -1)
            {
                ...
                do
                {
                    ...
                    if (TryGetFirstValueAtomic(data, key, out tempItem, out tempIt))
                    {
                        ...
                        do
                        {
                            nextPtrs[idx] = data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine];
                        } while (Interlocked.CompareExchange(
                                     ref data->firstFreeTLS[threadIndex * FNativeHashMapData.IntsPerCacheLine], idx,
        ...
        public static unsafe void AddAtomicMulti(FNativeHashMapData* data, TKey key, TValue item, int threadIndex)
        {
            // Allocate an entry from the free list
            int idx = AllocEntry(data, threadIndex);
        ...
        public static unsafe bool TryAdd(FNativeHashMapData* data, TKey key, TValue item, bool isMultiHashMap,
            Allocator allocation)
        {
            ...
            if (data->allocatedIndexLength >= data->keyCapacity && data->firstFreeTLS[0] < 0)
            {
                ...
                {
                    if (data->firstFreeTLS[tls * FNativeHashMapData.IntsPerCacheLine] >= 0)
                    {
                        idx = data->firstFreeTLS[tls * FNativeHashMapData.IntsPerCacheLine];
                        nextPtrs = (int*)data->next;
                        data->firstFreeTLS[tls * FNativeHashMapData.IntsPerCacheLine] = nextPtrs[idx];
                        nextPtrs[idx] = -1;
                ...
                {
                    int newCap = FNativeHashMapData.GrowCapacity(data->keyCapacity);
                    FNativeHashMapData.ReallocateHashMap<TKey, TValue>(data, newCap,
                        FNativeHashMapData.GetBucketSize(newCap), allocation);
                }
            }
        ...
        public static unsafe void Remove(FNativeHashMapData* data, TKey key, bool isMultiHashMap)
        {
        ...
        public static unsafe void Remove(FNativeHashMapData* data, FNativeMultiHashMapIterator<TKey> it)
        {
            // First find the slot based on the hash
            int* buckets = (int*)data->buckets;
            int* nextPtrs = (int*)data->next;
        ...
        unsafe public static void RemoveKeyValue<TValueEQ>(FNativeHashMapData* data, TKey key, TValueEQ value)
        ...
        public static unsafe bool TryGetFirstValueAtomic(FNativeHashMapData* data, TKey key, out TValue item,
            out FNativeMultiHashMapIterator<TKey> it)
        {
            it.key = key;
        ...
        public static unsafe bool TryGetNextValueAtomic(FNativeHashMapData* data, out TValue item,
            ref FNativeMultiHashMapIterator<TKey> it)
        {
        ...
        public static unsafe bool SetValue(FNativeHashMapData* data, ref FNativeMultiHashMapIterator<TKey> it,
            ref TValue item)
        {
    ...
    [StructLayout(LayoutKind.Sequential)]
    [NativeContainer]
    [DebuggerTypeProxy(typeof(FNativeHashMapDebuggerTypeProxy<,>))]
    public unsafe struct FNativeHashMap<TKey, TValue> : IDisposable
        where TKey : struct, IEquatable<TKey>
        where TValue : struct
    {
        [NativeDisableUnsafePtrRestriction]
        FNativeHashMapData* m_Buffer;
        ...
        public FNativeHashMap(int capacity, Allocator allocator)
            : this(capacity, allocator, 2)
        {
        }

        FNativeHashMap(int capacity, Allocator allocator, int disposeSentinelStackDepth)
        {
            FCollectionHelper.CheckIsUnmanaged<TKey>();
            FCollectionHelper.CheckIsUnmanaged<TValue>();
            ...
            FNativeHashMapData.AllocateHashMap<TKey, TValue>(capacity, capacity * 2, allocator, out m_Buffer);
        ...
        public int Length
        {
            get
            {
                ...
                FNativeHashMapData* data = m_Buffer;
                int* nextPtrs = (int*)data->next;
                int freeListSize = 0;
                for (int tls = 0; tls < JobsUtility.MaxJobThreadCount; ++tls)
                    for (int freeIdx = data->firstFreeTLS[tls * FNativeHashMapData.IntsPerCacheLine];
                        freeIdx >= 0;
                        freeIdx = nextPtrs[freeIdx])
                        ++freeListSize;
                return System.Math.Min(data->keyCapacity, data->allocatedIndexLength) - freeListSize;
            }
        }
        ...
        public int Capacity
        {
            ...
            set
            {
                ...
                FNativeHashMapData* data = m_Buffer;
                FNativeHashMapData.ReallocateHashMap<TKey, TValue>(data, value, FNativeHashMapData.GetBucketSize(value),
                    m_AllocatorLabel);
            }
        }
        ...
        void Deallocate()
        {
            FNativeHashMapData.DeallocateHashMap(m_Buffer, m_AllocatorLabel);
            m_Buffer = null;
        }
        ...
        {
            public FNativeHashMap<TKey, TValue> Container;
        ...
        public NativeArray<TKey> GetKeyArray(Allocator allocator)
        {
            ...
            FNativeHashMapData.GetKeyArray(m_Buffer, result);
            return result;
        }
        ...
        public NativeArray<TValue> GetValueArray(Allocator allocator)
        {
            ...
            FNativeHashMapData.GetValueArray(m_Buffer, result);
            return result;
        }
        ...
        {
            [NativeDisableUnsafePtrRestriction]
            internal FNativeHashMapData* m_Buffer;
    ...
    [StructLayout(LayoutKind.Sequential)]
    [NativeContainer]
    [DebuggerTypeProxy(typeof(FNativeMultiHashMapDebuggerTypeProxy<,>))]
    public unsafe struct FNativeMultiHashMap<TKey, TValue> : IDisposable
        where TKey : struct, IEquatable<TKey>
        where TValue : struct
    {
        [NativeDisableUnsafePtrRestriction] internal FNativeHashMapData* m_Buffer;
        ...
        public FNativeMultiHashMap(int capacity, Allocator allocator)
            : this(capacity, allocator, 2)
        {
        }

        FNativeMultiHashMap(int capacity, Allocator allocator, int disposeSentinelStackDepth)
        {
            m_AllocatorLabel = allocator;
            // Bucket size if bigger to reduce collisions
            FNativeHashMapData.AllocateHashMap<TKey, TValue>(capacity, capacity * 2, allocator, out m_Buffer);
        ...
        public void Remove(FNativeMultiHashMapIterator<TKey> it)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            AtomicSafetyHandle.CheckWriteAndThrow(m_Safety);
#endif
        ...
        public bool TryGetFirstValue(TKey key, out TValue item, out FNativeMultiHashMapIterator<TKey> it)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
#endif
        ...
        public bool TryGetNextValue(out TValue item, ref FNativeMultiHashMapIterator<TKey> it)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
#endif
        ...
        public bool SetValue(TValue item, FNativeMultiHashMapIterator<TKey> it)
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            AtomicSafetyHandle.CheckWriteAndThrow(m_Safety);
#endif
        ...
        struct DisposeJob : IJob
        {
            public FNativeMultiHashMap<TKey, TValue> Container;
        ...
        {
            internal FNativeMultiHashMap<TKey, TValue> hashmap;
            internal TKey key;
            internal bool isFirst;

            TValue value;
            FNativeMultiHashMapIterator<TKey> iterator;
    ...
    public static class JobNativeMultiHashMapUniqueHashExtensions
    {
        ...
        {
            [ReadOnly] public FNativeMultiHashMap<int, int> HashMap;
            public TJob JobData;
                ...
                while (true)
                {
                    ...
                    for (int i = begin; i < end; i++)
                    {
                        ...
                        while (entryIndex != -1)
                        {
                            ...
                            FNativeMultiHashMapIterator<int> it;
                            ...
                            {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                                var startIndex = System.Math.Min(firstValue, value);
                                var lastIndex = System.Math.Max(firstValue, value);
        ...
        public static unsafe JobHandle Schedule<TJob>(this TJob jobData, FNativeMultiHashMap<int, int> hashMap,
    ...
    public static class JobNativeMultiHashMapVisitKeyValue
    {
        internal struct NativeMultiHashMapVisitKeyValueJobStruct<TJob, TKey, TValue>
            ...
            where TValue : struct
        {
            [ReadOnly] public FNativeMultiHashMap<TKey, TValue> HashMap;
            public TJob JobData;
        ...
        public static unsafe JobHandle Schedule<TJob, TKey, TValue>(this TJob jobData, FNativeMultiHashMap<TKey, TValue> hashMap,
            int minIndicesPerJobCount, JobHandle dependsOn = new JobHandle())
            where TJob : struct, IJobNativeMultiHashMapVisitKeyValue<TKey, TValue>
            where TKey : struct, IEquatable<TKey>
            where TValue : struct
    ...
    public static class NativeHashMapExtensions
    {
        ...
        public static Tuple<NativeArray<TKey>, int> GetUniqueKeyArray<TKey, TValue>(this FNativeMultiHashMap<TKey, TValue> hashMap, Allocator allocator) where TKey : struct, IEquatable<TKey>, IComparable<TKey> where TValue : struct
        {
