...
{
    /// <summary>
    /// FNativeList is a replica of NativeList provided by Unity.Collections for the purpose of increased stability of the asset package.
    /// Unity frequently changes the Collections package, breaking stuff. This replica always remains the same no matter which version used.
    /// Is identical to the NativeList. Just that it will not be affected by Unity version changes.
    /// No one wants compile errors due to version changes!
    /// </summary>
    /// <typeparam name="T">The type of the elements in the list.</typeparam>
    [StructLayout(LayoutKind.Sequential)]
    [NativeContainer]
    [DebuggerDisplay("Length = {Length}")]
    [DebuggerTypeProxy(typeof(FNativeListDebugView<>))]
    public unsafe struct FNativeList<T> : IEnumerable<T>, IDisposable
        where T : struct
    {
        ...
        internal FUnsafeList* m_ListData;
        ...
        public FNativeList(Allocator allocator)
            : this(1, allocator, 2)
        {
        }
        ...
        /// <param name="allocator">A member of the
        /// [Unity.Collections.Allocator](https://docs.unity3d.com/ScriptReference/Unity.Collections.Allocator.html) enumeration.</param>
        public FNativeList(int initialCapacity, Allocator allocator)
            : this(initialCapacity, allocator, 2)
        {
        }

        FNativeList(int initialCapacity, Allocator allocator, int disposeSentinelStackDepth)
        {
...
#endif
            m_ListData = FUnsafeList.Create(UnsafeUtility.SizeOf<T>(), UnsafeUtility.AlignOf<T>(), initialCapacity, allocator);
        ...
        void Deallocate()
        {
            FUnsafeList.Destroy(m_ListData);
            m_ListData = null;
        }
        ...
        {
            public FNativeList<T> Container;
        ...
        public static implicit operator NativeArray<T>(FNativeList<T> nativeList)
        {
    ...
    sealed class FNativeListDebugView<T> where T : struct
    {
        FNativeList<T> m_Array;

        public FNativeListDebugView(FNativeList<T> array)
        {
            m_Array = array;
        }
...
{
    /// <summary>
    /// Utilities for unsafe access to a <see cref="FNativeList{T}"/>.
    /// </summary>
    public unsafe static class NativeListUnsafeUtility
    {
        ...
        public static void* GetUnsafePtr<T>(this FNativeList<T> list) where T : struct
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
        ...
        public static unsafe void* GetUnsafeReadOnlyPtr<T>(this FNativeList<T> list) where T : struct
        {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
        ...
        public static AtomicSafetyHandle GetAtomicSafetyHandle<T>(ref FNativeList<T> list) where T : struct
        {
        ...
        public static void* GetInternalListDataPtrUnchecked<T>(ref FNativeList<T> list) where T : struct
        {
