...
{
    unsafe struct FNativeQueueBlockHeader
    {
        public FNativeQueueBlockHeader* m_NextBlock;
        public int m_NumItems;
        public int m_NumReaders;
    }

    [StructLayout(LayoutKind.Sequential)]
    internal unsafe struct FNativeQueueBlockPoolData
    {
        ...
        public FNativeQueueBlockHeader* AllocateBlock()
        {
            ...
            FNativeQueueBlockHeader* checkBlock = (FNativeQueueBlockHeader*)m_FirstBlock;
            FNativeQueueBlockHeader* block;
            ...
            do
            {
                ...
                if (block == null)
                {
                    ...
                    block = (FNativeQueueBlockHeader*)UnsafeUtility.Malloc(m_BlockSize, 16, Allocator.Persistent);
                    return block;
                }

                checkBlock = (FNativeQueueBlockHeader*)Interlocked.CompareExchange(ref m_FirstBlock, (IntPtr)block->m_NextBlock, (IntPtr)block);
            }
        ...
        public void FreeBlock(FNativeQueueBlockHeader* block)
        {
            ...
            FNativeQueueBlockHeader* checkBlock = (FNativeQueueBlockHeader*)m_FirstBlock;
            FNativeQueueBlockHeader* nextPtr;

            do
            {
                nextPtr = checkBlock;
                block->m_NextBlock = checkBlock;
                checkBlock = (FNativeQueueBlockHeader*)Interlocked.CompareExchange(ref m_FirstBlock, (IntPtr)block, (IntPtr)checkBlock);
            }
            while (checkBlock != nextPtr);
        }
    }

    internal unsafe static class FNativeQueueBlockPool
    {
        static FNativeQueueBlockPoolData data;

        public static FNativeQueueBlockPoolData* QueueBlockPool
        {
            get
            {
                if (data.m_NumBlocks == 0)
                {
                    ...
                    FNativeQueueBlockHeader* prev = null;

                    for (int i = 0; i < data.m_MaxBlocks; ++i)
                    {
                        FNativeQueueBlockHeader* block = (FNativeQueueBlockHeader*)UnsafeUtility.Malloc(FNativeQueueBlockPoolData.m_BlockSize, 16, Allocator.Persistent);
                ...
                return (FNativeQueueBlockPoolData*)UnsafeUtility.AddressOf<FNativeQueueBlockPoolData>(ref data);
            }
        }
        ...
        static void OnDomainUnload(object sender, EventArgs e)
        {
            ...
            {
                FNativeQueueBlockHeader* block = (FNativeQueueBlockHeader*)data.m_FirstBlock;
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal unsafe struct FNativeQueueData
    {
        ...
        internal FNativeQueueBlockHeader* GetCurrentWriteBlockTLS(int threadIndex)
        {
            var data = (FNativeQueueBlockHeader**)&m_CurrentWriteBlockTLS[threadIndex * JobsUtility.CacheLineSize];
            return *data;
        }

        internal void SetCurrentWriteBlockTLS(int threadIndex, FNativeQueueBlockHeader* currentWriteBlock)
        {
            var data = (FNativeQueueBlockHeader**)&m_CurrentWriteBlockTLS[threadIndex * JobsUtility.CacheLineSize];
            *data = currentWriteBlock;
        }

        public static FNativeQueueBlockHeader* AllocateWriteBlockMT<T>(FNativeQueueData* data, FNativeQueueBlockPoolData* pool, int threadIndex) where T : struct
        {
            FNativeQueueBlockHeader* currentWriteBlock = data->GetCurrentWriteBlockTLS(threadIndex);
            ...
            if (currentWriteBlock == null)
            {
                ...
                FNativeQueueBlockHeader* prevLast = (FNativeQueueBlockHeader*)Interlocked.Exchange(ref data->m_LastBlock, (IntPtr)currentWriteBlock);
        ...
        public static FNativeQueueBlockHeader* GetFirstBlock(FNativeQueueData* data, FNativeQueueBlockPoolData* pool)
        {
            FNativeQueueBlockHeader* firstBlock = (FNativeQueueBlockHeader*)data->m_FirstBlock;
        ...
        public unsafe static void Release(FNativeQueueBlockHeader* block, FNativeQueueBlockPoolData* pool)
        {
        ...
        public unsafe static void AllocateQueue<T>(Allocator label, out FNativeQueueData* outBuf) where T : struct
        {
            var queueDataSize = FCollectionHelper.Align(UnsafeUtility.SizeOf<FNativeQueueData>(), JobsUtility.CacheLineSize);

            var data = (FNativeQueueData*)UnsafeUtility.Malloc(
                  queueDataSize
            ...
            data->m_LastBlock  = IntPtr.Zero;
            data->m_MaxItems   = (FNativeQueueBlockPoolData.m_BlockSize - UnsafeUtility.SizeOf<FNativeQueueBlockHeader>()) / UnsafeUtility.SizeOf<T>();
        ...
        public unsafe static void DeallocateQueue(FNativeQueueData* data, FNativeQueueBlockPoolData* pool, Allocator allocation)
        {
            FNativeQueueBlockHeader* firstBlock = (FNativeQueueBlockHeader*)data->m_FirstBlock;

            while (firstBlock != null)
            {
                FNativeQueueBlockHeader* next = firstBlock->m_NextBlock;
    ...
    [StructLayout(LayoutKind.Sequential)]
    [NativeContainer]
    unsafe public struct FNativeQueue<T> : IDisposable
        where T : struct
    {
        [NativeDisableUnsafePtrRestriction]
        FNativeQueueData* m_Buffer;

        [NativeDisableUnsafePtrRestriction]
        FNativeQueueBlockPoolData* m_QueuePool;
        ...
        public unsafe FNativeQueue(Allocator label)
        {
            FCollectionHelper.CheckIsUnmanaged<T>();

            m_QueuePool = FNativeQueueBlockPool.QueueBlockPool;
            m_AllocatorLabel = label;

            FNativeQueueData.AllocateQueue<T>(label, out m_Buffer);
        ...
        unsafe public int Count
        {
            get
            {
                ...
                for (FNativeQueueBlockHeader* block = (FNativeQueueBlockHeader*)m_Buffer->m_FirstBlock; block != null; block = block->m_NextBlock)
        ...
        {
            get { return FNativeQueueBlockPool.QueueBlockPool->m_MaxBlocks; }
            set { Interlocked.Exchange(ref FNativeQueueBlockPool.QueueBlockPool->m_MaxBlocks, value); }
        }

        static public int MemoryBlockSize
        {
            get { return FNativeQueueBlockPoolData.m_BlockSize; }
        }
        ...
        unsafe public T Peek()
        {
            ...
            AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
#endif
            FNativeQueueBlockHeader* firstBlock = (FNativeQueueBlockHeader*)m_Buffer->m_FirstBlock;
...
#endif
            FNativeQueueBlockHeader* writeBlock = FNativeQueueData.AllocateWriteBlockMT<T>(m_Buffer, m_QueuePool, 0);
        ...
        unsafe public bool TryDequeue(out T item)
        {
            ...
            AtomicSafetyHandle.CheckWriteAndThrow(m_Safety);
#endif
            FNativeQueueBlockHeader* firstBlock = FNativeQueueData.GetFirstBlock(m_Buffer, m_QueuePool);
            ...
            FNativeQueueData.Release(firstBlock, m_QueuePool);
        ...
        void Deallocate()
        {
            FNativeQueueData.DeallocateQueue(m_Buffer, m_QueuePool, m_AllocatorLabel);
            m_Buffer = null;
        }
        ...
        {
            public FNativeQueue<T> Container;
        ...
        {
            [NativeDisableUnsafePtrRestriction]
            internal FNativeQueueData* m_Buffer;

            [NativeDisableUnsafePtrRestriction]
            internal FNativeQueueBlockPoolData* m_QueuePool;
            ...
            unsafe public void Enqueue(T entry)
            {
                ...
                AtomicSafetyHandle.CheckWriteAndThrow(m_Safety);
#endif
                FNativeQueueBlockHeader* writeBlock = FNativeQueueData.AllocateWriteBlockMT<T>(m_Buffer, m_QueuePool, m_ThreadIndex);
                UnsafeUtility.WriteArrayElement(writeBlock + 1, writeBlock->m_NumItems, entry);
                ++writeBlock->m_NumItems;
            }
        }
