...
{
	[BurstCompile]
	public struct CrystallicMultiColor_Calculation : IJob
	{
		...
		public FNativeList<Color32> colorArray;
		...
		public void Init(Mesh crystal, int width, float surface = 0.5f)
		{
			...
			colorArray = new FNativeList<Color32>(0, Allocator.Persistent);
		...
		public void Execute()
		{
			...
			Vector3 vertexoffset = new Vector3(voxelSize, voxelSize, voxelSize) * 0.5f;
			for (int index = 0; index < blocks; index++)
			{
				...
				Vector3 vertexpos = new Vector3(fx2, fy2, fz2);

				Color32 color = new Color32(255, 255, 255, 255);
				if (texturedata_UV3.IsCreated)
				{
					color.r = (byte)texturedata_UV3._PeekVoxelId(vertexpos.x, vertexpos.y, vertexpos.z, 10, 0);
				}

				if (texturedata_UV4.IsCreated)
				{
					color.g = (byte)texturedata_UV4._PeekVoxelId(vertexpos.x, vertexpos.y, vertexpos.z, 10, 0);
				}

				if (texturedata_UV5.IsCreated)
				{
					color.b = (byte)texturedata_UV5._PeekVoxelId(vertexpos.x, vertexpos.y, vertexpos.z, 10, 0);
				}

				
				if (texturedata_UV6.IsCreated)
				{
					float dimensionScale = texturedata_UV6._PeekVoxelId(vertexpos.x, vertexpos.y, vertexpos.z, 10, 0)/256.0f;
				...
				for (int v = 0; v < mesh_vertcount; v++)
				{
					...
					verticeArray.Add(vertexpos + vertex + vertexoffset);
