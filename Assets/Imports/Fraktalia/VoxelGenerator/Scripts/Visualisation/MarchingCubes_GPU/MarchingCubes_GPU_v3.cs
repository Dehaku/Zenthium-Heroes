...
{
	public unsafe class MarchingCubes_GPU_v3 : UniformGridVisualHull_SingleStep
	{
		...
		protected override void beginCalculation(int m, int index, float cellSize, float voxelSize, float startX, float startY, float startZ)
		{
			...
			//VoxelMeshes[index].voxelMesh.bounds = bound;
		}
		...
		protected override float GetChecksum()
		{
			...
			return base.GetChecksum() + Cell_Subdivision * 1000 + width * 100 + NumCores * 10 + SurfacePoint + SurfaceDimension
