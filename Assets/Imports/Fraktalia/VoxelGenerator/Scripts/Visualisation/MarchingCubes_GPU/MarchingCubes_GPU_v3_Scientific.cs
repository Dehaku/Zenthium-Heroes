...
{
	public unsafe class MarchingCubes_GPU_v3_Scientific : UniformGridVisualHull_SingleStep
	{
		...
		NativeCreateUniformGrid_V2_Scientific[] calculators;
		...
		public float[] Histogramm;
		public Bounds VisibleBoundary = new Bounds(new Vector3(0.5f,0.5f,0.5f), new Vector3(0.5f, 0.5f, 0.5f));
		...
		protected override void Initialize()
		{
			...
			CreateHistogramm();
			NativeHistogramm = new NativeArray<float>(Histogramm, Allocator.Persistent);
		...
		protected override void initializeCalculators()
		{
			...
			calculators = new NativeCreateUniformGrid_V2_Scientific[NumCores];
		...
		protected override void beginCalculation(int m, int index, float cellSize, float voxelSize, float startX, float startY, float startZ)
		{
			...
			calculators[m].Histogramm = NativeHistogramm;
			calculators[m].VisibleBoundary = VisibleBoundary;
		...
		{
			if (NativeHistogramm.IsCreated) NativeHistogramm.Dispose();
			base.cleanup();
		}
		...
		protected override float GetChecksum()
		{
			...
			if (NativeHistogramm.IsCreated)
			{
				for (int i = 0; i < 256; i++)
				{
					details += Histogramm[i];
				}
			}

			

			return base.GetChecksum() + Cell_Subdivision * 1000 + width * 100 + NumCores * 10 + SurfacePoint + SurfaceDimension + VisibleBoundary.size.sqrMagnitude + VisibleBoundary.center.sqrMagnitude
		...
		public void CreateHistogramm()
		{
			...
			for (int i = 0; i < 256; i++)
			{
				float value = HistogrammCurve.Evaluate((float)i / 256.0f) * 256f;
				Histogramm[i] = Mathf.Clamp(value, 0, 256);

				if (NativeHistogramm.IsCreated)
				{
					NativeHistogramm[i] = Histogramm[i];
				}
			}
