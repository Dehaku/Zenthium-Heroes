...
namespace Fraktalia.VoxelGen.Visualisation
{
	...
	[BurstCompile]
	public struct MarchingCubes_MultiTexture_Calculation : IJob
	{
	...
	[BurstCompile]
	public struct MarchingCubes_MultiTexture_Calculation_Improved : IJob
	{
		[NativeDisableParallelForRestriction]
		public NativeVoxelTree data;
		...
		[NativeDisableParallelForRestriction]
		private NativeArray<float> Cube;
		private NativeArray<Vector3> CubeNormals;


		[NativeDisableParallelForRestriction]
		private NativeArray<Vector3> EdgeVertex;
		private NativeArray<Vector3> EdgeNormals;
		public Vector3 positionoffset;
		public float voxelSize;
		public float cellSize;
		...
		{
			float fx = positionoffset.x + x * voxelSize;
			float fy = positionoffset.y + y * voxelSize;
			float fz = positionoffset.z + z * voxelSize;
		...
		void VoxelValueWithGradient(Vector3Int i, int index)
		{
			Vector3Int i_n = i - Vector3Int.one;
			Vector3Int i_p = i + Vector3Int.one;
			...
			Cube[index] = v;
			CubeNormals[index] = new Vector3(v_px - v_nx, v_py - v_ny, v_pz - v_nz);
		}

		void FillCube(int x, int y, int z)
		{
			VoxelValueWithGradient(new Vector3Int(x, y, z), 0);
			VoxelValueWithGradient(new Vector3Int(x + 1, y, z), 1);
			VoxelValueWithGradient(new Vector3Int(x + 1, y + 1, z), 2);
			VoxelValueWithGradient(new Vector3Int(x, y + 1, z), 3);
			VoxelValueWithGradient(new Vector3Int(x, y, z + 1), 4);
			VoxelValueWithGradient(new Vector3Int(x + 1, y, z + 1), 5);
			VoxelValueWithGradient(new Vector3Int(x + 1, y + 1, z + 1), 6);
			VoxelValueWithGradient(new Vector3Int(x, y + 1, z + 1), 7);
		}
			...
			for (int index = 0; index < blocks; index++)
			{
				...
				for (i = 0; i < 12; i++)
				{
					...
					if ((edgeFlags & (1 << i)) != 0)
					{
						...
						Vector3 sample1 = CubeNormals[edgeconnection];
						Vector3 sample2 = CubeNormals[EdgeConnection[i * 2 + 1]];
				
						EdgeNormals[i] = -Vector3.Lerp(sample1, sample2, offset);
				...
				for (i = 0; i < 5; i++)
				{
					...
					for (j = 0; j < 3; j++)
					{
						vert = TriangleConnectionTable[flagIndex * 16 + 3 * i + j];
						Vector3 vertex = (EdgeVertex[vert] * voxelSize) + positionoffset;
						Vector3 normal = EdgeNormals[vert];
						if (texturedata_UV3.IsCreated)
						{
							float texturevalue = texturedata_UV3._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, Shrink) * TexturePowerUV3;
							uv3Array.Add(new Vector2(texturevalue, texturevalue));
		...
		public void CleanUp()
		{
			...
			if (Cube.IsCreated) Cube.Dispose();
			if (CubeNormals.IsCreated) CubeNormals.Dispose();
			if (EdgeVertex.IsCreated) EdgeVertex.Dispose();
			if (EdgeNormals.IsCreated) EdgeNormals.Dispose();
			if (dictionary.IsCreated) dictionary.Dispose();
			if (dictionaryKeys.IsCreated) dictionaryKeys.Dispose();
			if (triNormals.IsCreated) triNormals.Dispose();
			if (colorArray.IsCreated) colorArray.Dispose();
