...
namespace Fraktalia.VoxelGen.Visualisation
{
	...
	[BurstCompile]
	public struct MarchingCubes_AtlasTexture_Calculation : IJob
	{
		...
		public FNativeList<Vector2> atlasArray;
		...
		public void Init(int width, float surface = 0.5f)
		{
			...
			atlasArray = new FNativeList<Vector2>(0, Allocator.Persistent);
			...
			for (int index = 0; index < blocks; index++)
			{
				...
				for (i = 0; i < 5; i++)
				{
					...
					for (j = 0; j < 3; j++)
					{
						...
						{
							float texturevalue = texturedata_UV3._PeekVoxelId(vertex.x, vertex.y, vertex.z, 10, Shrink);
							atlasArray.Add(new Vector2(texturevalue, texturevalue));
						}					
		...
		public void CleanUp()
		{
			...
			if (atlasArray.IsCreated) atlasArray.Dispose();
